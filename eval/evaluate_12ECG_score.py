...
def compute_auc(labels, outputs):
    ...
    for k in range(num_classes):
        ...
        thresholds = np.unique(outputs[:, k]) # remove all repeated probs for class k, return a sorted prob .
        thresholds = np.append(thresholds, thresholds[-1]+1) # add a 2.0 at the end
        thresholds = thresholds[::-1]  # reverse the order
        ...
        fn[0] = np.sum(labels[:, k]==1) # number of samples positive on class k
        tn[0] = np.sum(labels[:, k]==0) # number of samples negative on class k

        # Find the indices that result in sorted output values.
        idx = np.argsort(outputs[:, k])[::-1] # in decreasing order
